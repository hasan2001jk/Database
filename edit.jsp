		...
		<!--center-wrapper wraps this card-> article -->	
			<div class = "article">
			 ...
			 <h4 class="article-sub-subtitle" id="article_subtitle">Редактирование базы данных</h4>
				<form method="post" action="ServletEdit" >
				<!-- forma starts here -->
					<div class="forma">
						...
						<!-- labels -->
						<div class="vibor">
						<label for="strings"><span class="strings-text">По строкам</span><input type="radio" id="strings" name="choose" checked value="with_string" onchange="check()"></label>
						<label for="cell"><span class="cell-text">По ячейкам</span><input type="radio" id="cell" name="choose" value="with_cells" onchange="check()"></label>
						</div>
						...
						<label for="library-card" id="library-card-label"><span id="library-text">Введите номер читательского билета :</span><input type="text" id="library-card" class="library-card" placeholder="Номер билета" name="lib_card"></label>
	...
	<!-- body-wrapper finishes  here-->
				<span >
						 <%if(request.getAttribute("sms") != null){
							out.println("<h6 class='servlet' >"+request.getAttribute("sms").toString()+"</h6>");
							}%>
					</span>	
...
function change(){
		...
		function fun1() {
			  ...
			  if(options[sel].value=="books"){
				  ...
				  remove_one_two=document.getElementById('library-card');
				  ...
				  std_name_placeholder.name="std_name";
				  /*-------------------------------------------------*/
		...
		function check(){
			...
			if(strings.checked){
				var tex_t = document.getElementById('article_subtitle');
				tex_t.textContent="Редактирование Базы Данных ";
			}else{
				var tex = document.getElementById('article_subtitle');
				tex.textContent="Выберите ID строки и заполните необходимую ячейку ";
			}
